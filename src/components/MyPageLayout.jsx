    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
          ...
          if (window.innerWidth <= 768 && sidebarElement && toggleButton) {
            if (isSidebarOpen && !sidebarElement.contains(event.target) && !toggleButton.contains(event.target)) {
              setIsSidebarOpen(false);
            }
          }
        };
                ...
                <header className="flex-shrink-0 bg-black bg-opacity-90 backdrop-blur-md border-b border-gray-800">
                    <div className="container mx-auto px-4">
                        <div className="flex items-center justify-between h-16">
                            <button id="sidebar-toggle" className="md:hidden text-white" onClick={toggleSidebar}>
                                <FontAwesomeIcon icon={faBars} className="text-xl" />
                            </button>
                            <h2 className="orbitron text-2xl font-bold text-white hidden md:block">
                                {getPageTitle()}
                            </h2>
                            <h2 className={`orbitron text-xl font-bold text-white md:hidden ${isSidebarOpen ? 'hidden' : 'block'}`}>
                                {getPageTitle()}
                            </h2>
                            <div className="flex items-center space-x-4 ml-auto">
                                <button className="text-gray-400 hover:text-white"><FontAwesomeIcon icon={faBell} /></button>
                                <div className="flex items-center space-x-2">
                                    <img src="https://via.placeholder.com/40/A8C5E6/FFFFFF?text=User" alt="Profile" className="w-8 h-8 rounded-full" />
                                    <span className="text-sm font-medium">김TCP</span>
                                </div>
                                <button className="px-4 py-2 text-sm border border-gray-600 rounded-lg hover:border-gray-400 transition-colors">로그아웃</button>
                            </div>
                        </div>
                    </div>
                </header>
