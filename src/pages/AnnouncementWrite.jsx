...
function AnnouncementWrite() {
  ...
  const getCategoryName = (cat) => {
    const names = {'general': '일반공지', 'event': '행사안내', 'recruitment': '모집공고', 'rule': '규정안내', 'urgent': '긴급공지'};
    return names[cat] || '카테고리';
  };
  
  const getCategoryColor = (cat) => {
    const colors = {'general': 'blue', 'event': 'green', 'recruitment': 'purple', 'rule': 'pink', 'urgent': 'red'};
    ...
    return (
      <>
        <header className="mb-8">
          <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-${getCategoryColor(category)}-400 to-${getCategoryColor(category)}-500 text-black`}>{getCategoryName(category)}</span>
          <h1 className="orbitron text-4xl font-bold mt-4 gradient-text">{title}</h1>
        ...
        <div className="bg-gray-800 rounded-lg p-8">
          <div className="text-gray-200 whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: safeHtml }} />
        </div>
      </>
    );
  };
    ...
    <main className="pt-20 pb-16 min-h-screen">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="orbitron text-4xl md:text-5xl font-bold gradient-text mb-2">공지사항 작성</h1>
          <p className="text-gray-400">TCP 동아리 회원들에게 전달할 중요한 소식을 작성해주세요.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="form-card rounded-xl p-8">
          {/* 제목 입력 */}
          <div className="mb-8">
            <label htmlFor="title" className="block text-lg font-semibold mb-3 text-white text-left">
            ...
            <div className="flex justify-between items-center mt-2">
              <p className="text-sm text-gray-400">명확하고 간결한 제목을 작성해주세요.</p>
              <span className={`char-counter ${title.length > 90 ? 'warning' : ''}`}>{title.length}/100</span>
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              {['general', 'event', 'recruitment', 'rule', 'urgent'].map(cat => (
                <div
                  key={cat}
                  ...
                  data-category={cat}
                >
                  <i className={`mr-1 fas ${cat === 'general' ? 'fa-bullhorn' : cat === 'event' ? 'fa-calendar-alt' : cat === 'recruitment' ? 'fa-user-plus' : cat === 'rule' ? 'fa-gavel' : 'fa-exclamation-triangle'}`}></i>
                  {getCategoryName(cat)}
                </div>
              ))}
            </div>
          ...
          {/* 날짜 설정 */}
          <div className="mb-8">
            <label htmlFor="date" className="block text-lg font-semibold mb-3 text-white text-left">
            ...
            />
            <p className="text-sm text-gray-400 mt-2 text-left">공지사항이 게시될 날짜를 선택해주세요.</p>
          </div>

          {/* 내용 작성 */}
          <div className="mb-8">
            <label htmlFor="content" className="block text-lg font-semibold mb-3 text-white text-left">
              <i className="fas fa-align-left text-pink-400 mr-2"></i>내용
            </label>
            
            {/* 텍스트 서식 도구바 */}
            <div className="flex flex-wrap gap-2 mb-3 p-3 bg-gray-800 rounded-lg">
                <button type="button" className="toolbar-btn" onClick={() => formatText('bold')} title="굵게"><i className="fas fa-bold"></i></button>
                <button type="button" className="toolbar-btn" onClick={() => formatText('italic')} title="기울임"><i className="fas fa-italic"></i></button>
                <button type="button" className="toolbar-btn" onClick={() => formatText('underline')} title="밑줄"><i className="fas fa-underline"></i></button>
                <div className="border-l border-gray-600 mx-2"></div>
                <button type="button" className="toolbar-btn" onClick={() => insertText('• ')} title="불릿 포인트"><i className="fas fa-list-ul"></i></button>
                <button type="button" className="toolbar-btn" onClick={() => insertText('1. ')} title="번호 목록"><i className="fas fa-list-ol"></i></button>
                <div className="border-l border-gray-600 mx-2"></div>
                <button type="button" className="toolbar-btn" onClick={() => insertText('📌 ')} title="핀 이모지">📌</button>
                <button type="button" className="toolbar-btn" onClick={() => insertText('⚠️ ')} title="경고 이모지">⚠️</button>
                <button type="button" className="toolbar-btn" onClick={() => insertText('🎉 ')} title="축하 이모지">🎉</button>
            </div>
...
- 마무리 인사"
            ...
            <div className="flex justify-between items-center mt-2">
              <p className="text-sm text-gray-400">마크다운 문법을 지원합니다. (**, *, _, 등)</p>
              <span className={`char-counter ${content.length > 2800 ? 'warning' : ''}`}>{content.length}/3000</span>
            </div>
          </div>

          {/* 작성자 정보 */}
          <div className="mb-8">
            <label htmlFor="author" className="block text-lg font-semibold mb-3 text-white text-left">
      ...
      {/* 미리보기 모달 */}
      {isPreviewModalOpen && (
        <div id="previewModal" className="fixed inset-0 preview-modal flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-xl max-w-4xl w-full mx-4 max-h-screen overflow-y-auto">
            <div className="sticky top-0 bg-gray-900 px-6 py-4 border-b border-gray-700 flex justify-between items-center">
              <h2 className="orbitron text-xl font-bold gradient-text">미리보기</h2>
              <button onClick={() => setIsPreviewModalOpen(false)} className="text-gray-400 hover:text-white">
      ...
      {/* 성공 메시지 모달 */}
      {isSuccessModalOpen && (
        <div id="successModal" className="fixed inset-0 preview-modal flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-xl p-8 mx-4 text-center">
            ...
            <h3 className="orbitron text-xl font-bold gradient-text mb-2">게시 완료!</h3>
            <p className="text-gray-300 mb-6">공지사항이 성공적으로 게시되었습니다.</p>
            <button onClick={() => navigate('/announcement')} className="btn-primary px-6 py-2 rounded-lg font-medium text-white">
              공지사항 목록 보기
            </button>
          </div>
        </div>
