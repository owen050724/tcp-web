      ...
      {filteredStudies.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredStudies.map((study) => (
            <div
              key={study.id}
              className="study-card p-6 rounded-xl card-hover"
            >
              <div className="flex items-center justify-between mb-4">
                <span
                  className={`status-badge ${study.status === 'ongoing' ? 'bg-blue-600' : 'bg-green-600'}`}
                >
                  <FontAwesomeIcon
                    icon={
                      study.status === 'ongoing' ? faPlayCircle : faCheckCircle
                    }
                    className="mr-2"
                  />
              ...
              <h4 className="orbitron text-xl font-bold text-white mb-2">
                {study.title}
              </h4>
