...
const StepIndicator = ({ currentStep }) => (
  <div className="step-indicator">
    <div className={`step ${currentStep >= 1 ? 'active' : ''}`} id="step-1">1</div>
    <div className={`step-connector ${currentStep > 1 ? 'completed' : ''}`} id="connector-1"></div>
    <div className={`step ${currentStep >= 2 ? 'active' : ''}`} id="step-2">2</div>
    <div className={`step-connector ${currentStep > 2 ? 'completed' : ''}`} id="connector-2"></div>
    <div className={`step ${currentStep >= 3 ? 'active' : ''}`} id="step-3">3</div>
  </div>
);
...
const Withdraw = () => {
  ...
  useEffect(() => {
    ...
    setIsFinalButtonEnabled(allChecked);
  }, [confirmations]);
    ...
    <div className="container mx-auto max-w-4xl">
      ...
      {currentStep === 1 && (
        <section id="withdrawal-step-1">
          {/* Step 1 content from HTML, converted to JSX */}
          <div className="widget-card p-8 rounded-xl">
                  ...
                  <div className="warning-list">
                      <ul>
                          <li>
                              ...
                              <span><strong>프로필 정보:</strong> 닉네임, 프로필 사진, 자기소개, 기술 스택 등 모든 개인 정보</span>
                          </li>
                          <li>
                              <i className="fas fa-history"></i>
                              <span><strong>활동 기록:</strong> 참여한 스터디, 팀 프로젝트, 대회 참가 이력 등 모든 활동 데이터</span>
                          </li>
                          <li>
                              <i className="fas fa-comments"></i>
                              <span><strong>커뮤니케이션:</strong> 작성한 게시글, 댓글, 메시지 등 모든 소통 기록</span>
                          </li>
                          <li>
                              <i className="fas fa-trophy"></i>
                              <span><strong>성과 기록:</strong> 획득한 배지, 포인트, 랭킹 등 모든 성취 데이터</span>
                          </li>
                      </ul>
                  </div>
              </div>
                  ...
                  <div className="warning-list">
                      <ul>
                          <li>
                              ...
                              <span><strong>재가입 제한:</strong> 탈퇴 후 30일간 동일한 이메일로 재가입이 불가능합니다</span>
                          </li>
                          <li>
                              <i className="fas fa-database"></i>
                              <span><strong>데이터 복구 불가:</strong> 삭제된 모든 데이터는 복구할 수 없으며, 백업도 제공되지 않습니다</span>
                          </li>
                          <li>
                              <i className="fas fa-users"></i>
                              <span><strong>팀 활동 중단:</strong> 현재 참여 중인 모든 팀 활동에서 자동으로 제외됩니다</span>
                          </li>
                          <li>
                              <i className="fas fa-link"></i>
                              <span><strong>외부 연동 해제:</strong> GitHub, 포트폴리오 등 연동된 모든 외부 서비스 연결이 해제됩니다</span>
                          </li>
                      </ul>
                  </div>
              </div>
              ...
              <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => goToStep(2)} className="btn-danger">
           ...
           <div className="widget-card p-8 rounded-xl">
              ...
              <div className="max-w-md mx-auto">
                  <div className="mb-6">
                      ...
                      <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="editable" placeholder="현재 사용 중인 비밀번호를 입력하세요" />
                      {passwordError && <div className="text-red-400 text-sm mt-2"><i className="fas fa-exclamation-circle mr-1"></i> {passwordError}</div>}
                  </div>
        ...
        <section id="withdrawal-step-3">
          ...
          <div className="widget-card p-8 rounded-xl">
              ...
              <div className="max-w-2xl mx-auto">
                  <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 mb-6">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                              ...
                              <span className="text-white ml-2 font-medium">Admin Kim</span>
                          </div>
                          <div>
                              <span className="text-gray-400">이메일:</span>
                              <span className="text-white ml-2 font-medium">kimtcp@seoultech.ac.kr</span>
                          </div>
                          <div>
                              <span className="text-gray-400">가입일:</span>
                              <span className="text-white ml-2 font-medium">2022년 3월 15일</span>
                          </div>
                          <div>
                              <span className="text-gray-400">활동 기간:</span>
                              <span className="text-white ml-2 font-medium">2년 3개월</span>
                          </div>
                      </div>
                  </div>
                  ...
                  <div className="space-y-4 mb-8">
                      <div className="checkbox-container">
                          <input type="checkbox" id="dataLoss" checked={confirmations.dataLoss} onChange={handleConfirmationChange} />
                          <label htmlFor="dataLoss" className="text-gray-300">모든 개인 데이터(프로필, 활동 기록, 작성 글 등)가 영구적으로 삭제되며, 복구가 불가능함을 이해했습니다.</label>
                      </div>
                      <div className="checkbox-container">
                          <input type="checkbox" id="reregisterLimit" checked={confirmations.reregisterLimit} onChange={handleConfirmationChange} />
                          <label htmlFor="reregisterLimit" className="text-gray-300">탈퇴 후 30일간 동일한 이메일로 재가입이 불가능함을 이해했습니다.</label>
                      </div>
                      <div className="checkbox-container">
                          <input type="checkbox" id="teamExit" checked={confirmations.teamExit} onChange={handleConfirmationChange} />
                          <label htmlFor="teamExit" className="text-gray-300">현재 참여 중인 모든 팀 활동에서 자동으로 제외되며, 이로 인한 팀원들의 불편함을 이해했습니다.</label>
                      </div>
                      <div className="checkbox-container">
                          <input type="checkbox" id="final" checked={confirmations.final} onChange={handleConfirmationChange} />
                          <label htmlFor="final" className="text-gray-300 font-semibold">위의 모든 내용을 숙지했으며, <span className="text-red-400"><span className="orbitron">TCP</span> 계정을 영구적으로 삭제</span>하는 것에 동의합니다.</label>
                      </div>
                  </div>
              ...
              <div className="flex justify-between items-center">
                  ...
                  <button onClick={finalWithdraw} id="final-withdraw-btn" className={`btn-danger ${!isFinalButtonEnabled && 'opacity-50 cursor-not-allowed'}`} disabled={!isFinalButtonEnabled}>
      ...
      {currentStep === 4 && (
        <section id="withdrawal-success" className="success-animation">
          {/* Success content */}
          <div className="widget-card p-8 rounded-xl text-center">
